...
from _skimage2.util._array_api import xp_swapaxes
...
# >>>>>>>>>>>>> UPTOHERE <<<<<<<<<<<<<<<<<
    ...
    def test_yuv(self):
        ...
        assert_array_almost_equal(rgb2ypbpr(rgb), xp.asarray([[[1, 0, 0]]]))
        assert_array_almost_equal(rgb2ycbcr(rgb), xp.asarray([[[235, 128, 128]]]))
        assert_array_almost_equal(rgb2ydbdr(rgb), xp.asarray([[[1, 0, 0]]]))
        rgb = xp.asarray([[[0.0, 1.0, 0.0]]])
        assert_array_almost_equal(
            rgb2yuv(rgb), xp.asarray([[[0.587, -0.28886916, -0.51496512]]])
        )
        assert_array_almost_equal(
            rgb2yiq(rgb), xp.asarray([[[0.587, -0.27455667, -0.52273617]]])
        )
        assert_array_almost_equal(
            rgb2ypbpr(rgb), xp.asarray([[[0.587, -0.331264, -0.418688]]])
        )
        assert_array_almost_equal(
            rgb2ycbcr(rgb), xp.asarray([[[144.553, 53.797, 34.214]]])
        )
...
def test_gray2rgb():
    ...
    xp_assert_equal(w, expected_output)
    ...
    xp_assert_equal(y[..., 0], x)
    xp_assert_equal(y[0, 0, :], xp.asarray([0., 0, 0]))

    x = xp.asarray([[0, 128, 255]], dtype=xp.uint8)
    z = gray2rgb(x)

    assert z.shape == (1, 3, 3)
    xp_assert_equal(z[..., 0], x)
...
def test_gray2rgba(shape, channel_axis):
    ...
    xp_assert_equal(rgba.shape, shape[:new_axis_loc] + (4,) + shape[new_axis_loc:])
    ...
    for channel in range(3):
        xp_assert_equal(rgba[slice_at_axis(channel, axis=new_axis_loc)], img)

    # Alpha channel check
    value = rgba[slice_at_axis(3, axis=new_axis_loc)]
    xp_assert_equal(value, xp.full_like(value, fill_value=1.0))
...
def test_gray2rgb_channel_axis(shape, channel_axis):
    ...
    xp_assert_equal(rgb.shape, shape[:new_axis_loc] + (3,) + shape[new_axis_loc:])
...
def test_gray2rgba_alpha():
    ...
    xp_assert_equal(rgba[..., :3], gray2rgb(img))
    xp_assert_equal(rgba[..., 3], xp.full(rgba.shape[:-1], fill_value=1.0))
    ...
    xp_assert_equal(rgba[..., :3], gray2rgb(img))
    xp_assert_equal(rgba[..., 3], xp.full(rgba.shape[:-1], fill_value=alpha))

    # Array
    alpha = xp.asarray(np.random.random((5, 5)))
    rgba = gray2rgba(img, alpha)
    ...
    with expected_warnings(["alpha cannot be safely cast to image dtype"]):
        ...
        xp_assert_equal(rgba[..., :3], gray2rgb(img_u8))

    # Invalid shape
    alpha = xp.asarray(np.random.random((5, 5, 1)))
...
def test_rgb2hsv_dtypes(dtype):
    ...
    hsv = xp.reshape(rgb2hsv(rgb), (-1, 3))
    float_dtype = _supported_float_type(dtype)
